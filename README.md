...
Это сайт сети ресторанов Star Burger. Здесь можно заказать превосходные бургеры с доставкой на дом. Демо-версия сайта расположена по адресу: [star-burger.alexwolf.ru](https://star-burger.alexwolf.ru/)
...
## Yandex JavaScript API и HTTP Геокодер

Для корректной работы функций определения местоположения и расстояния между адресами потребуется получить ключ в [кабинете разработчика](https://developer.tech.yandex.ru/)
...
![такую картинку](https://dvmn.org/filer/canonical/1594651900/687/)
...
На сервере должны быть запущены сервисы `nginx` и `postgres`.
